...
class PDFChatbot:
    ...
    def save_to_cache(self, identifier: str) -> None:
        ...
        try:
            ...
            if self.chunk_embeddings is not None:
                ...
                np.save(f"{cache_path}_embeddings.npy", self.chunk_embeddings.cpu().numpy() if hasattr(self.chunk_embeddings, 'cpu') else self.chunk_embeddings)
    ...
    def load_from_cache(self, identifier: str) -> bool:
        ...
        try:
            ...
            # Load embeddings and convert back to tensor if necessary for the model
            loaded_embeddings = np.load(f"{cache_path}_embeddings.npy")
            import torch
            self.chunk_embeddings = torch.from_numpy(loaded_embeddings)
    ...
    def generate_answer(self, question: str, context_chunks: List[Dict], client: OpenAI) -> str:
        ...
        context_str = ""
        # Using a set to collect unique sources for attribution
        sources = set()
        ...
        prompt = f"""You are a helpful and precise AI assistant that answers questions based *only* on the provided context from PDF documents.
...
6.  Maintain a professional, objective, and factual tone. Avoid conversational filler.

Answer:"""

        try:
            response = client.chat.completions.create(
                model="gpt-4o-mini", # Using the smaller, faster model for cost-effectiveness
                messages=[
            ...
            return f"Error generating response from OpenAI: {e}. Please check your OpenAI API key, model access, and internet connection."


# --- STREAMLIT UI ---

def get_openai_client():
    """Gets the OpenAI client, prioritizing environment variables over Streamlit secrets."""
    # Using st.secrets is the recommended way for Streamlit Cloud deployments
    api_key = st.secrets.get("OPENAI_API_KEY") 
    
    if not api_key:
        st.warning("🚨 OpenAI API key not found! Please set it as a Streamlit secret (in .streamlit/secrets.toml) or an environment variable named `OPENAI_API_KEY`.")
        return None
    
    try:
        return OpenAI(api_key=api_key)
    except Exception as e:
        st.error(f"Error initializing OpenAI client: {e}. Check if your API key is valid.")
        return None
...
def main():
    ...
    if openai_client is None:
        st.stop() # Stop execution if no valid OpenAI client can be initialized
    ...
    with st.sidebar:
        ...
        if uploaded_files:
            ...
            # Using hashlib.sha256 for a more robust hash, especially for many files
        ...
        if st.session_state.last_used_identifier == identifier and cache_info_display.get('is_valid', False) and st.session_state.pdfs_processed:
            # If current files match last processed and cache is valid and already processed, disable button
            # Or perhaps change text to "Re-Process PDFs"
        ...
        if question := st.chat_input("Ask about the content of the PDFs..."):
            ...
            with st.chat_message("assistant"):
                ...
                with st.spinner("Searching and generating answer..."):
                    ...
                    answer = st.session_state.chatbot.generate_answer(question, relevant_chunks, openai_client)
