...
class PDFChatbot:
    ...
    def __init__(self, model_name: str = 'all-MiniLM-L6-v2'):
        ...
        @st.cache_resource
        def load_embedding_model_with_retry(name, max_retries=3):
            """Load embedding model with retry logic for rate limiting."""
            for attempt in range(max_retries):
                try:
                    ...
                    # Add cache directory for model to avoid repeated downloads
                    ...
                    if "429" in str(e):
                        ...
                        st.error(f"HTTP Error loading model: {e}")
                        break
                    ...
                    if attempt == max_retries - 1:
                        ...
                        st.warning(f"Error loading model: {e}. Retrying in {wait_time} seconds...")
                        time.sleep(wait_time)
            
            # If all attempts failed, try fallback
            return self._load_fallback_model()
        
        self.embedding_model = load_embedding_model_with_retry(model_name)
        if self.embedding_model:
            self.model_device = self.embedding_model.device
        else:
            st.error("Could not load any embedding model.")
            st.stop()

    def _load_fallback_model(self):
        """Fallback to download model manually or use alternative."""
        try:
            st.info("Attempting to load model with offline mode...")
            # Try to load in offline mode (if model was previously downloaded)
            cache_folder = os.path.join(os.getcwd(), "model_cache")
            model = SentenceTransformer(
                'all-MiniLM-L6-v2',
                cache_folder=cache_folder,
            ...
            st.success("✅ Model loaded in offline mode")
            return model
        except:
            try:
                st.info("Trying alternative model...")
                # Try a different model that might be more available
                model = SentenceTransformer('paraphrase-MiniLM-L6-v2')
                st.success("✅ Alternative model loaded")
                return model
            except Exception as e:
                st.error(f"All model loading attempts failed: {e}")
                return None
    ...
    def create_chunks_and_embeddings(self) -> None:
        ...
        # Generate semantic embeddings with retry logic
        chunk_texts = [chunk['text'] for chunk in self.text_chunks]
        try:
            ...
            for i in range(0, len(chunk_texts), batch_size):
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt == max_retries - 1:
                            st.error(f"Failed to encode batch {i//batch_size + 1}: {e}")
                            raise e
                        else:
                            wait_time = (2 ** attempt) * 2
                            st.warning(f"Retrying batch {i//batch_size + 1} in {wait_time} seconds...")
                            time.sleep(wait_time)
                
                # Show progress
                progress = min((i + batch_size) / len(chunk_texts), 1.0)
                st.progress(progress, text=f"Processing embeddings... {int(progress * 100)}%")
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt == max_retries - 1:
                        st.error(f"Failed to encode question: {e}")
                        return []
                    else:
                        wait_time = (2 ** attempt) * 2
                        time.sleep(wait_time)
...
def main():
    ...
    with st.expander("ℹ️ About Rate Limiting"):
        st.info("""
        This app uses Hugging Face models which may occasionally hit rate limits. 
        The app includes automatic retry logic with exponential backoff to handle this.
        If you continue to see rate limiting errors, try:
        
        1. Waiting a few minutes before retrying
        2. Using the cached results if available
